...
def handle(doc, state: str):
    HazlerDocumentEventHandler(doc, state).handle()
...
class HazlerDocumentEventHandler:
    ...
    def execute_workflow_if_applicable(self, wf_name, wf_trigger_config):
        #  {"event":"New","doctype":"ToDo"}
        event = wf_trigger_config.get("event")
        doctype = wf_trigger_config.get("doctype")

        if not (self.doctype == doctype and self.state == STATE_EVENT_MAP[event]):
            return
        context = frappe._dict({"trigger_config": wf_trigger_config, "doc": self.doc})
        if event == "On Change":
